                                ...
                                progress:(BADownloadProgress)progress
{
    ...
    scc.requestSerializer = [AFHTTPRequestSerializer serializer];
    scc.responseSerializer = [AFHTTPResponseSerializer serializer];
    ...
    if (type == BAHttpRequestTypeGet)
    {
        sessionTask = [BANetManagerShare.sessionManager GET:URLString parameters:parameters  progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            if (encoding) {
                encode = CFStringConvertEncodingToNSStringEncoding(CFStringConvertIANACharSetNameToEncoding((__bridge CFStringRef)encoding));
            } else {
                encode = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGBK_95);/* some sane default? */
            }
